...
function bc_loss(N, β)
    ...
    out_original = M(a)
    loss = (β/N)*sum(out_original.^2)
    return loss
end

#add losses
function total_loss(N, space = 0.01, β = 1000)
...
function plot_surface()
    ...
    heatmap(1:size(data,1),
        ...
        xlabel = "X", ylabel = "Y",
        title = "2D Full Square Fitted Solution")

    savefig("2d_fullsq_fitted.png")
end
